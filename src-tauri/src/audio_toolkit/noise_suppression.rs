...
impl NoiseSuppressionLevel {
    ...
    ///
    /// These values are applied as a floor on the denoiser's gain values,
    /// preventing it from suppressing too aggressively which can cause
...
///
/// The noise suppressor processes audio frame-by-frame (480 samples at 16kHz = 30ms)
/// before it reaches VAD. This improves VAD accuracy in noisy environments by
/// removing background noise that could trigger false speech detections.
///
/// # Usage
/// ```no_run
/// let mut ns = NoiseSuppressor::new(NoiseSuppressionLevel::Medium);
/// let clean_frame = ns.process(&noisy_frame);
/// ```
///
/// # Performance
/// Processing adds ~0.1-0.3ms latency per 30ms frame on modern hardware.
/// Memory overhead is minimal (~200KB for internal buffers).
...
impl NoiseSuppressor {
    ...
    ///
    /// Input samples should be in the range [-1.0, 1.0] (normalized f32).
    /// Internally, the denoiser works in i16 range, so we scale before/after.
    ...
    /// This can be used as an additional signal for voice activity detection.
    ...
    /// to avoid carry-over from previous sessions.
    ...
    /// This takes effect on the next frame processed.
...
mod tests {
    ...
    fn test_noise_suppressor_reset() {
        let mut ns = NoiseSuppressor::new(NoiseSuppressionLevel::Low);
        let frame = vec![0.1; NOISE_SUPPRESSION_FRAME_SIZE];

        // Process first frame (discarded)
        ns.process(&frame);
