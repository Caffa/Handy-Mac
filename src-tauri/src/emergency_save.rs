...
impl EmergencyBackup {
    ...
    pub fn complete_recording(&self) -> Option<PathBuf> {
        ...
        if let Some(ref path) = backup_path {
            ...
            debug!("Emergency backup removed after successful completion: {:?}", path);
        }
    ...
    pub fn panic_save(&self, samples: &[f32], sample_rate: u32) {
        ...
        let new_name = backup_path.file_name()
            .and_then(|n| n.to_str())
    ...
    fn write_wav_backup(&self, path: &PathBuf, samples: &[f32], sample_rate: u32) -> Result<(), String> {
                ...
                if let Some(filename) = entry.file_name().to_str() {
                    ...
                    if filename.starts_with(EMERGENCY_PREFIX) && 
                       (filename.contains("inprogress") || 
                        ...
                        let recovered_name = recovered_name.replace(".wav", &format!("_{}.wav", timestamp));
...
fn write_wav_header(writer: &mut BufWriter<File>, sample_count: u32, sample_rate: u32) -> Result<(), String> {
    ...
    writer.write_all(&file_size.to_le_bytes()).map_err(|e| e.to_string())?;
    writer.write_all(b"WAVE").map_err(|e| e.to_string())?;
    
    // fmt chunk
    writer.write_all(b"fmt ").map_err(|e| e.to_string())?;
    writer.write_all(&16u32.to_le_bytes()).map_err(|e| e.to_string())?; // chunk size
    writer.write_all(&1u16.to_le_bytes()).map_err(|e| e.to_string())?; // PCM format
