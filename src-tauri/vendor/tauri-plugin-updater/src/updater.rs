...
impl Update {
    ...
    pub async fn download<C: FnMut(usize, Option<u64>), D: FnOnce()>(
        ...
        #[cfg(not(feature = "dangerous-skip-verify"))]
        verify_signature(&buffer, &self.signature, &self.config.pubkey)?;

        #[cfg(feature = "dangerous-skip-verify")]
        log::warn!("update signature verification skipped (dangerous-skip-verify feature enabled)");
