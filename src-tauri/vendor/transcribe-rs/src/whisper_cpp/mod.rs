...
pub struct WhisperInferenceParams {
    ...
    /// Force single segment output. Useful for short audio clips (push-to-talk)
    /// where the audio is known to be a single utterance. Skips the segmentation
    /// step, significantly reducing overhead for short recordings.
    /// Default `false` (whisper.cpp decides segmentation).
    pub single_segment: bool,

    /// Use greedy decoding instead of beam search.
    /// Greedy decoding is significantly faster (no beam search overhead) with
    /// minimal accuracy loss for short, clear speech (typical push-to-talk use).
    /// Default `true` (greedy) for best speed-accuracy tradeoff.
    /// Set `false` to use beam search with beam_size=3 for marginally better
    /// accuracy on noisy or long-form audio.
    pub use_greedy: bool,
}
...
impl WhisperEngine {
    ...
    fn infer(
        ...
        let strategy = if params.use_greedy {
