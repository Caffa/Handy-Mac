...
- When the audio worker thread finishes, `stream.pause()` is called before `drop(stream)`, but `pause()` is **asynchronous** — it only *requests* that CoreAudio stop calling the callback
